...
from getlocation.models import Country, Divisions, District, Upazila

class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        # Create or get the country
        country, created = Country.objects.get_or_create(name="বাংলাদেশ")

        # Define division and district data with corresponding upazilas
        division_districts_upazilas = {
            "ঢাকা": {
        ...
        # Populate Divisions, Districts, and Upazilas
        for division_name, districts_upazilas in division_districts_upazilas.items():
            # Create Division instance and associate it with the country
            division, created = Divisions.objects.get_or_create(name=division_name, country=country)

            # Iterate over districts and their corresponding upazilas
            for district_name, upazila_names in districts_upazilas.items():
                # Create District instance and associate it with the division
                district, created = District.objects.get_or_create(division=division, name=district_name)

                # Create Upazila instances and associate them with the district
                for upazila_name in upazila_names:
                    upazila, created = Upazila.objects.get_or_create(district=district, name=upazila_name)
                    if created:
                        self.stdout.write(self.style.SUCCESS(f"Upazila '{upazila_name}' created successfully in '{district_name}', '{division_name}'."))
                    else:
                        self.stdout.write(self.style.WARNING(f"Upazila '{upazila_name}' already exists in '{district_name}', '{division_name}'."))

        self.stdout.write(self.style.SUCCESS("Population of Divisions, Districts, and Upazilas completed."))
